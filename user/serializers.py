...
class TeacherRegistrationSerializer(serializers.ModelSerializer):
    ...
    @staticmethod
    def _validate_m2m_count(value, max_count, field_name):
        if len(value) > max_count:
            raise serializers.ValidationError(
                _("You can select a maximum of {count} {field}.").format(
                    count=max_count, field=field_name
                )
            )
        if not value:
            raise serializers.ValidationError(
                _("Please select at least one {field}.").format(field=field_name)
            )
        return value

    def validate_subjects(self, value):
        return self._validate_m2m_count(
            value, getattr(settings, "MAX_TEACHER_SUBJECTS", 2), "subjects"
        )

    def validate_categories(self, value):
        return self._validate_m2m_count(
            value, getattr(settings, "MAX_TEACHER_CATEGORIES", 2), "categories"
        )

    def validate_languages(self, value):
        return self._validate_m2m_count(
            value, getattr(settings, "MAX_TEACHER_LANGUAGES", 3), "languages"
        )
...
class TeacherCabinetSerializer(serializers.ModelSerializer):
    ...
    city_read = serializers.StringRelatedField(source="city", read_only=True)
    categories_read = serializers.StringRelatedField(
        source="categories", many=True, read_only=True
    )
    subjects_read = serializers.StringRelatedField(
