...
try:
    ...
    raise ImportError(
        "Could not import models from 'user' app. Check your project structure and INSTALLED_APPS."
    )
...
class Lesson(models.Model):
    ...
    def update_status_if_expired(self):
        ...
        if self.end_time and now > self.end_time:
            expired = True
        elif not self.end_time and now > (self.start_time + timedelta(hours=1)):
            expired = True
...
class Rating(models.Model):
    ...
    rating = models.PositiveSmallIntegerField(
        _("Rating"), validators=[MinValueValidator(1), MaxValueValidator(5)], default=3
    )
